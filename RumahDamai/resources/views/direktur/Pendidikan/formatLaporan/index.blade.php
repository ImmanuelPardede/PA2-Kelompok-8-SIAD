        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    @endif
                    <a href="{{ route('direktur.formatLaporan.create') }}" class="btn btn-success mb-3">Tambah Format Laporan</a>
                </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if ($formatLaporan->format_laporan)
                                            <a href="{{ route('direktur.formatLaporan.download', $formatLaporan->id) }}">{{ $formatLaporan->format_laporan }}</a>
                                        @else
                                            Data tidak tersedia
                                        @endif
                                    </td>
                                    <td>
                                        <a href="{{ route('direktur.formatLaporan.edit', $formatLaporan->id) }}"
                                            class="btn btn-warning">Edit</a>
                                        <form method="POST" id="deleteForm{{ $formatLaporan->id }}" class="d-inline"
                                            action="{{ route('direktur.formatLaporan.destroy', $formatLaporan->id) }}">
                                            @csrf
                                            @method('DELETE')
